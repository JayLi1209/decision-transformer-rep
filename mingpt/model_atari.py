"""
GPT model:
- the embedding has token encoding + positional encoding
- model is a uniform sequence of Transformer blocks
    - each transformer = MLP block + self-attention block
...
class GPTConfig:
    '''The base class (to be inherited on)'''
    embd_pdrop = 0.1
    resid_pdrop = 0.1
    attn_pdrop = 0.1
...
class Block(nn.Module):
    '''Transformer block: layer1 and layer2, attention block, and MLP'''
...
class GPT(nn.Module):
    ''' GPT Model, context size = block_size '''
   ...
   def configure_optimizers(self, train_config):
        """
        Some parameter needs decay over time (learning rate), some don't (weights).
        The method separates them.
        """
